...
const ProductDetailsScreen = ({ navigation, route }) => {
  ...
  const handleBuyNow = () => {
    const itemsToAddCount = Object.values(quantities).filter(
      qty => qty > 0,
    ).length;
  ...
  const activePricingArray = isWholesale
    ? product?.businessPricing
    : product?.retailPricing;
  const basePrice = getBasePrice(product, isWholesale);

  const totalAmount =
    activePricingArray?.reduce((sum, tier, idx) => {
  ...
  const otherItemsQty = otherItemsInCart.reduce(
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: Math.max(insets.top, 16) }]}>
        ...
        <View style={styles.headerActions}>
          ...
          >
            <MaterialIcons name="share" size={22} color={'#475569'} />
          </TouchableOpacity>
          <TouchableOpacity
      ...
      >
        ...
        <View style={styles.infoCard}>
          ...
          {(isWholesale ? product?.businessPricing : product?.retailPricing)
            ?.length > 0 && (
                    ...
                    >
                      <View style={[styles.cell, { flex: 1.5 }]}>
                        <Text style={styles.tierQty}>
                          {tier.unit ||
                            (idx === 0 ? 'Loose' : `Opt ${idx + 1}`)}
                          {tier.label ? (
                            <Text style={styles.tierLabelSub}>
                              {' '}
                              ({tier.label})
                            </Text>
                          ) : null}
                      ...
                      <View
                        style={[
                          styles.cell,
                          { flex: 1.2, alignItems: 'flex-end' },
                        ]}
                      >
                        <View
                          style={[
                            styles.tierQtySelector,
                            isActive && styles.tierQtySelectorActive,
                          ]}
                        >
                          <TouchableOpacity
          ...
          {totalAmount > 0 && (
            <View style={styles.selectionSummary}>
                    ...
                    return (
                      <View key={idx} style={styles.summaryChip}>
                        <Text style={styles.summaryChipText}>
                          {qty} × {tier?.unit || 'Units'} = ₹
                          {(qty * (tier?.price || 0)).toLocaleString()}
                        </Text>
                      </View>
                    );
                  })}
              ...
              <View style={styles.selectionTotalRow}>
                ...
                <Text style={styles.selectionTotalPrice}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={styles.totalPrice}>
                <Text style={styles.totalLabel}>
                  ...
                  {otherItemsQty + totalSelectedQty === 1 ? 'Item' : 'Items'} |{' '}
                </Text>
          ...
          >
            {relatedProducts.length > 0 ? (
              ...
              <Text style={[styles.descriptionText, { marginLeft: 0 }]}>
      ...
      >
        <TouchableOpacity style={styles.outlineBtn} onPress={handleAddToCart}>
          ...
          <Text style={styles.outlineBtnText}>
            {isWholesale ? 'Add to Bulk' : 'Add to Cart'}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.primaryBtn} onPress={handleBuyNow}>
          <Text style={styles.primaryBtnText}>
            {isWholesale ? 'Order Now' : 'Buy Now'}
          </Text>
          <MaterialIcons
            name={isWholesale ? 'shopping-cart' : 'arrow-forward'}
            size={20}
            color="#fff"
          />
        </TouchableOpacity>
