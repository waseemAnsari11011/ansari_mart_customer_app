...
const ProductListingScreen = ({ navigation, route }) => {
  ...
  const isWholesaleRoute =
    route?.params?.isWholesale ?? user?.type === 'Business';
  ...
  const fetchProducts = async (pageNumber = 1, search = searchQuery) => {
    try {
      let url = `/products?userType=${
        isWholesaleRoute ? 'business' : 'retail'
      }`;
          ...
          <View style={styles.productFooter}>
            <View>
              <Text style={styles.productPrice}>
                ...
                {(() => {
                  const pricingArr = isWholesaleRoute
                    ? item.businessPricing
                    : item.retailPricing;
                  const unitToDisplay =
                    pricingArr && pricingArr.length > 0
                      ? pricingArr[0].unit
                      : null;
                  return unitToDisplay ? (
                    <Text style={styles.unitText}> / {unitToDisplay}</Text>
                  ) : null;
                })()}
              </Text>
            </View>
