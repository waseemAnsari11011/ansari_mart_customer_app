...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput, Dimensions, StatusBar, Alert, ActivityIndicator, Platform, PermissionsAndroid } from 'react-native';
import MaterialIcons from 'react-native-vector-icons/MaterialIcons';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useDispatch } from 'react-redux';
import { updateAddressAsync } from '../../redux/slices/addressSlice';
...
const EditAddressScreen = ({ navigation, route }) => {
    ...
    const [selectedLabel, setSelectedLabel] = useState(address?.label?.toLowerCase() || 'home');
    ...
    const [formData, setFormData] = useState({
        ...
        state: address?.state || '',
        city: address?.city || '',
        houseNo: address?.address?.split(', ')[0] || '',
        roadName: address?.address?.split(', ').slice(1).join(', ') || '',
            ...
            Geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    Geolocation.getCurrentPosition(
                        ...
                        { enableHighAccuracy: false, timeout: 5000, maximumAge: 1000 * 60 * 60 * 24 }
                    );
                },
    ...
    return (
        <View style={[styles.container, isDarkMode && styles.darkContainer]}>
            ...
            <View style={[styles.header, isDarkMode && styles.darkHeader, { paddingTop: Math.max(insets.top, 16) }]}>
                ...
                <Text style={[styles.headerTitle, isDarkMode && styles.darkText]}>Edit Address</Text>
            ...
            <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: 150 + insets.bottom }]}>
                ...
                >
                    ...
                    <Text style={styles.locationBtnText}>
                        {loadingLocation ? (locationStatus || 'Fetching...') : 'Use Current Location'}
                    </Text>
                </TouchableOpacity>
                ...
                <View style={styles.form}>
                    ...
                    <View style={styles.inputGroup}>
                        ...
                        <TextInput
                            style={[styles.input, styles.textArea, isDarkMode && styles.darkCard]}
                            placeholder="Industrial Area Phase II, Near Metro Station"
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingBottom: 16, backgroundColor: 'rgba(255, 255, 255, 0.8)', borderBottomWidth: 1, borderBottomColor: 'rgba(226, 232, 240, 0.5)' },
    darkHeader: { backgroundColor: 'rgba(15, 23, 42, 0.8)', borderBottomColor: 'rgba(30, 41, 59, 0.5)' },
    ...
    darkButtonBg: { backgroundColor: '#1E293B' },
    darkText: { color: '#F1F5F9' },
    scrollContent: { paddingHorizontal: 20, paddingTop: 16 },
    locationBtn: { backgroundColor: 'rgba(46, 125, 50, 0.05)', paddingVertical: 14, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(46, 125, 50, 0.2)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 24 },
    locationBtnText: { color: '#2E7D32', fontWeight: 'bold', fontSize: 15 },
    form: { gap: 20 },
    label: { fontSize: 12, fontWeight: 'bold', color: '#64748B', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8, marginLeft: 4 },
    ...
    darkCard: { backgroundColor: '#1E293B', borderColor: '#334155', color: '#fff' },
    selectedRadio: { backgroundColor: '#2E7D32', borderColor: '#2E7D32' },
    radioText: { fontSize: 14, fontWeight: 'bold', color: '#1E293B' },
    ...
    prefix: { height: '100%', justifyContent: 'center', paddingHorizontal: 16, borderWidth: 1, borderColor: '#E2E8F0', borderRightWidth: 0, backgroundColor: '#F8FAFC', borderTopLeftRadius: 12, borderBottomLeftRadius: 12, position: 'absolute', left: 0, zIndex: 2 },
    darkPrefix: { backgroundColor: '#1E293B', borderColor: '#334155' },
    prefixText: { fontSize: 15, color: '#94A3B8', fontWeight: '500' },
    phoneInput: { flex: 1, paddingLeft: 65 },
    row: { flexDirection: 'row', gap: 12 },
    picker: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    pickerText: { fontSize: 15, color: '#0F172A' },
    textArea: { height: 100, textAlignVertical: 'top' },
    saveFooter: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(255, 255, 255, 0.9)', paddingHorizontal: 20, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#E2E8F0' },
    darkFooter: { backgroundColor: 'rgba(15, 23, 42, 0.9)', borderTopColor: '#334155' },
    saveBtn: { backgroundColor: '#2E7D32', paddingVertical: 16, borderRadius: 16, alignItems: 'center', shadowColor: '#2E7D32', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
