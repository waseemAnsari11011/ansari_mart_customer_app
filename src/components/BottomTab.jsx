...
const BottomTab = ({ activeTab, isWholesale = false, isDarkMode = false }) => {
    ...
    const animations = useRef(navItems.reduce((acc, item) => ({
        ...acc,
        [item.name]: new Animated.Value(1)
    }), {})).current;

    const translateVs = useRef(navItems.reduce((acc, item) => ({
        ...acc,
    ...
    const animateTab = (name) => {
        ...
        Object.keys(animations).forEach(key => {
            if (key !== name) {
                Animated.parallel([
                    Animated.spring(animations[key], { toValue: 1, useNativeDriver: true }),
                    Animated.spring(translateVs[key], { toValue: 0, useNativeDriver: true })
                ]).start();
            }
        });
    ...
    return (
        <View style={[
            styles.bottomNav,
            isDarkMode && styles.darkBottomNav,
                ...
                return (
                    ...
                    >
                        <Animated.View style={[
                            ...
                            isActive && isDarkMode && styles.darkActiveIconWrapper,
                            animatedStyle
                        ]}>
                            <Icon
                                name={isActive && item.name === 'Profile' ? 'person' : item.icon}
                                size={isActive ? 24 : 22}
                                color={isActive ? '#fff' : (isDarkMode ? '#64748B' : '#aaa')}
                            />
                        ...
                        <Text style={[
                            ...
                            isActive && isDarkMode && { color: (isDarkMode ? '#FBBF24' : '#3E9400') }
                        ]}>
                            {item.label}
                        </Text>
                    </TouchableOpacity>
